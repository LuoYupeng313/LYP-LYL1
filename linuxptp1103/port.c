...
capable:
	if (p->asCapable == NOT_CAPABLE) {
		...
		/* Send gPTP-capable signaling message to advertise capability */
		if (p->gptp_capable_transmit) {
			port_tx_gptp_capable(p);
		}
	}
	return 1;
...
not_capable:
	...
	port_notify_event(p, NOTIFY_CMLDS);
	/* Send gPTP-capable signaling to indicate loss of capability */
	if (p->gptp_capable_transmit) {
		port_tx_gptp_capable(p);
	}
	return 0;
...
void port_disable(struct port *p)
{
	...
	/* Reset sync interval monitoring */
	p->last_sync_recv_time = tmv_zero();
	p->sync_interval_exceed_count = 0;
...
int port_initialize(struct port *p)
{
	...
	/* Initialize sync interval monitoring */
	p->last_sync_recv_time = tmv_zero();
	p->sync_interval_exceed_count = 0;
...
void process_sync(struct port *p, struct ptp_message *m)
{
	...
	/* Check sync receive interval for abnormal gaps */
	if (msg_sots_valid(m)) {
		tmv_t current_sync_time = m->hwts.ts;
		if (!tmv_is_zero(p->last_sync_recv_time)) {
			tmv_t interval = tmv_sub(current_sync_time, p->last_sync_recv_time);
			int64_t interval_ns = tmv_to_nanoseconds(interval);
			
			/* Check if interval exceeds 500ms (500,000,000 ns) */
			if (interval_ns > 500000000LL) {
				p->sync_interval_exceed_count++;
				pr_warning("%s: sync receive interval exceeded 500ms: %" PRId64 "ns, count=%d", 
					   p->log_name, interval_ns, p->sync_interval_exceed_count);
				
				/* Set flag to reset servo if 5 consecutive exceeds detected */
				if (p->sync_interval_exceed_count >= 5) {
					pr_info("%s: 5 consecutive sync intervals exceeded 500ms, requesting servo reset", p->log_name);
					clock_request_servo_reset(p->clock);
					/* Don't reset counter immediately - let normal interval reset it */
				}
			} else {
				/* Reset counter if interval is normal */
				p->sync_interval_exceed_count = 0;
			}
		}
		p->last_sync_recv_time = current_sync_time;
	}
...
static void port_p2p_transition(struct port *p, enum port_state next)
{
	...
	case PS_LISTENING:
		port_set_announce_tmo(p);
		port_set_delay_tmo(p);
		/* Send initial gPTP-capable signaling for 802.1AS/Automotive Profile */
		if (p->gptp_capable_transmit) {
			port_tx_gptp_capable(p);
		}
		break;
...
void port_link_status(void *ctx, int linkup, int ts_index)
{
	...
	if (p->link_status & link_state) {
		...
		pr_info("%s: link %s", p->log_name, linkup ? "up" : "down");
	}
...
static enum fsm_event bc_event(struct port *p, int fd_index)
{
	...
	case FD_SYNC_RX_TIMER:
		...
		/* Handle servo reset for noop slave when master disconnects */
		if (p->bmca == BMCA_NOOP && p->state == PS_SLAVE && fd_index == FD_SYNC_RX_TIMER) {
			pr_info("*** %s: sync receive timeout ***", p->log_name);
			clock_reset_servo_on_master_disconnect(p->clock);
		} else {
			pr_debug("*** %s: timeout detected but conditions not met - bmca=%d, state=%d, fd_index=%s ***", p->log_name, p->bmca, p->state, FD_SYNC_RX_TIMER ? "rx sync" : "announce");
		}
	...
	case FD_SYNC_TX_TIMER:
		...
		/* Check master restart detection flag before sending sync */
		if (clock_master_restart_detected(p->clock)) {
			pr_info("*** %s: stop transmitting sync due to master restart detection ***", 
				 p->log_name);
			return EV_NONE;
		} else {
			pr_info("*** %s: keep transmitting sync ***", p->log_name);
		}
	...
	case SYNC:
		process_sync(p, msg);
		if(p->state==PS_SLAVE && dds_domainNumber_get(p->clock)==1) {
		  sync_state_shm_update(slave_servo_stable_shm_get(p->clock), 1, 1);
		}
		break;
	case DELAY_REQ:
		       ...
		       struct clock *clock)
{
	...
	p->gptp_capable_transmit = config_get_int(cfg, p->name, "gptp_capable_transmit");
