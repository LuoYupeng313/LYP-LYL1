...
int process_signaling(struct port *p, struct ptp_message *m)
{
	...
	TAILQ_FOREACH(extra, &m->tlv_list, list) {
		...
		case TLV_ORGANIZATION_EXTENSION_DO_NOT_PROPAGATE:
			...
			/* Check for gPTP-capable TLV (IEEE 802.1 subtype 0x000001) */
			if (0 == memcmp(org->id, ieee8021_id, sizeof(ieee8021_id)) &&
			    org->subtype[0] == 0 && org->subtype[1] == 0 && org->subtype[2] == 1) {
				struct gptp_capable_tlv *gptp = (struct gptp_capable_tlv *) extra->tlv;
				pr_info("%s: received gPTP-capable signaling from %s",
					p->log_name, pid2str(&m->header.sourcePortIdentity));
				pr_debug("%s: peer gPTP-capable flags=0x%02x interval=%d",
					 p->log_name, gptp->flags, gptp->logGptpCapableMessageInterval);
				/* Store peer's gPTP capability - could be used for future logic */
			}
			break;
		}
	}
...
int port_tx_gptp_capable(struct port *p)
{
	struct gptp_capable_tlv *gptp;
	struct PortIdentity tpid;
	struct ptp_message *msg;
	struct tlv_extra *extra;
	int err;

	/* Send for IEEE 802.1AS (0x1) and Automotive Profile (0x2) */
	if (!port_is_ieee8021as(p) && p->transportSpecific != (2 << 4)) {
		pr_debug("%s: gPTP-capable TLV only for 802.1AS modes", p->log_name);
		return 0;
	}

	pr_debug("%s: sending gPTP-capable TLV (asCapable=%d, transportSpecific=0x%x)", 
		 p->log_name, port_capable(p), p->transportSpecific >> 4);
	...
	extra = msg_tlv_append(msg, sizeof(*gptp));
	if (!extra) {
		err = -1;
		goto out;
	}

	gptp = (struct gptp_capable_tlv *) extra->tlv;
	gptp->type = TLV_ORGANIZATION_EXTENSION_DO_NOT_PROPAGATE;
	gptp->length = sizeof(*gptp) - sizeof(gptp->type) - sizeof(gptp->length);
	memcpy(gptp->id, ieee8021_id, sizeof(ieee8021_id));
	gptp->subtype[0] = 0;
	gptp->subtype[1] = 0;
	gptp->subtype[2] = 1;  /* subtype 1 for gPTP-capable */
	gptp->flags = 0;
	gptp->logGptpCapableMessageInterval = 0x7F;  /* not specified */

	err = port_prepare_and_send(p, msg, TRANS_GENERAL);
	if (err) {
		pr_err("%s: send gPTP-capable signaling failed", p->log_name);
	} else {
		pr_info("%s: sent gPTP-capable signaling message", p->log_name);
	}
out:
	msg_put(msg);
	return err;
