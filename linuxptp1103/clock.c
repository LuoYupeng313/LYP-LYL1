...
struct clock {
	...
	struct servo_state_shm *servo_shm; /* Shared memory for servo state */
	int master_restart_detected; /* Flag indicating domain 0 master restart */
	struct master_restart_shm *master_restart_shm; /* Shared memory for master restart detection */
	struct slave_servo_stable_shm *slave_servo_stable_shm; /* Shared memory for slave servo stable state */
	int servo_reset_requested; /* Flag for sync interval exceed reset */
};
...
void clock_destroy(struct clock *c)
{
	...
	clock_destroy_servo_shared_memory(c);
	clock_destroy_master_restart_shared_memory(c);
	clock_destroy_slave_servo_stable_shared_memory(c);
			   ...
			   const char *phc_device)
{
	...
	c->servo_shm = NULL;
	c->master_restart_detected = 0;
	c->master_restart_shm = NULL;
	c->slave_servo_stable_shm = NULL;
  	c->servo_reset_requested = 0;
	
	if (clock_init_servo_shared_memory(c) < 0) {
		pr_err("Failed to initialize servo shared memory");
	}

	if (clock_init_master_restart_shared_memory(c) < 0) {
		pr_err("Failed to initialize master restart detection shared memory");
	}

	if (clock_init_slave_servo_stable_shared_memory(c) < 0) {
		pr_err("Failed to initialize slave servo stable shared memory");
	}
...
int clock_poll(struct clock *c)
{
	...
	/* Check for Hot-stanby restart */
	if (c->dds.domainNumber == 1 && !(c->dds.flags & DDS_SLAVE_ONLY)) {
		int restart_state = domain1_master_restart_detected();
		if(restart_state == 1){
       //wait 250ms for domain 1 slave send don't recieved the sync state to domain 0 master
       //the reason why set 250ms is that the sync interval is 125ms
			 usleep(125000+125000);
		}
	}
	/* Check for GM restart */
	if (c->dds.domainNumber == 0 && !(c->dds.flags & DDS_SLAVE_ONLY)) {
		int restart_state = domain0_master_restart_detected();
		int sync_received_state = 0;
		if(restart_state == 1){
			//wait 125ms + bitter for slave 100% received sync if GM single point restart
			usleep(125000+2000);
			int read_state = sync_state_shm_read(c->slave_servo_stable_shm, &sync_received_state);
			if(read_state == 0 && sync_received_state == 1){
				c->master_restart_detected = 1;//need servo in domain 1
        		pr_info("*** GM-Domain0-Master: GM single point restart ***");
			} else if(read_state == 0 && sync_received_state == 0){
        		c->master_restart_detected = 0;//forbide servo in domain 1
       			pr_info("*** GM-Domain0-Master: Complete vehicle restart ***");
      		}
			if(read_state == -1){
				pr_err("*** GM-Domain0-Master: read sync state failed, ERROR!!! ***");
			}
		}
		/* Write master restart detection to shared memory if available */
		if (c->master_restart_shm) {
			master_restart_shm_update(c->master_restart_shm, c->master_restart_detected, c->dds.domainNumber);
			if (c->master_restart_detected) {
				pr_debug("*** GM-Domain0-Master: GM single point restart, write to shm ***");
			}
		}
		
		/* Read slave servo stable state from shared memory if available */
		if (c->slave_servo_stable_shm) {
			int slave_servo_stable = 0;
			if (slave_servo_stable_shm_read(c->slave_servo_stable_shm, 1, &slave_servo_stable) == 0) {
				if (slave_servo_stable) {
					pr_debug("*** GM-Domain0-Master: GM single point restart recover, read from shm ***");
					c->master_restart_detected = 0;
				}
			}
		}
...
enum servo_state clock_synchronize(struct clock *c, tmv_t ingress, tmv_t origin)
{
	...
	adj = servo_sample(c->servo, offset, tmv_to_nanoseconds(ingress),
			   weight, &state);
	c->servo_state = state;
	/* Check if servo reset was requested due to sync interval exceed */
	if (c->servo_reset_requested) {
		pr_info("Executing servo reset due to sync interval exceed in clock_synchronize");
		clock_reset_servo_on_master_disconnect(c);
		c->servo_reset_requested = 0; /* Clear flag after use */
    	goto no_adjust;	
	}
	...
	if (c->dds.domainNumber == 0 && (c->dds.flags & DDS_SLAVE_ONLY)) {
		/* Domain 0: Write state to shared memory */
		servo_state_shm_update(c->servo_shm, state, 0);
	} else if (c->dds.domainNumber == 1 && (c->dds.flags & DDS_SLAVE_ONLY)) {
		/* Domain 1: Read state from domain 0 */
		enum servo_state domain0_state;
		int domain0_master_restart;
   
		/* master restart detection for GM */
		if (master_restart_shm_read(c->master_restart_shm, 0, &domain0_master_restart) == 0) {	
			if (domain0_master_restart == 1) {
        		//c->master_restart_detected = 1;
				if(state == SERVO_LOCKED_STABLE){
					/* Servo is stable, stop adjustments */
					skip_servo_update = 1;
          			//c->master_restart_detected = 0;	
					/* Write slave servo stable state to shared memory */
					slave_servo_stable_shm_update(c->slave_servo_stable_shm, 1, c->dds.domainNumber);
          			pr_info("*** GM-Domain1-slave: adjustment is stable, don't adjust, state turn Non-restart ***");
				}else {
					/* Continue adjustments until stable */
					skip_servo_update = 0;
          			slave_servo_stable_shm_update(c->slave_servo_stable_shm, 0, c->dds.domainNumber);
          			pr_info("*** GM-Domain1-slave: Restart, adjustment is not stable(s%d), keep adjust ***", state);
				}
			}else if(domain0_master_restart == 0){
				/* No restart detected */
				skip_servo_update = 1;
        		slave_servo_stable_shm_update(c->slave_servo_stable_shm, 0, c->dds.domainNumber);
				pr_info("*** GM-Domain1-slave: Non-restart state, don't adjust ***");
			}     
		} else if (servo_state_shm_read(c->servo_shm, 0, &domain0_state) == 0) {
		/* servo state detection for Hot-Stanby */ 		
			if (domain0_state != SERVO_UNLOCKED) {
				/* If domain 0 is not UNLOCKED, skip servo update */
				skip_servo_update = 1;
				pr_info("*** SLAVE-Domain1-slave: Skip servo update, domain 0 state is %d ***", domain0_state);
			} else {
				pr_info("*** SLAVE-Domain1-slave: Perform servo update, domain 0 state is UNLOCKED ***");
			}  
		} else {
			/* Could not read domain 0 both servo_state and monitor_state */
			/* First time read or not initialized yet, just skip */
			skip_servo_update = 1;
			pr_err("*** GM/HotS-Domain1-slave: need start GM-domain0-master or HotStandby-domain0-slave ***");
		}
				
	}

	if (skip_servo_update) {
		goto no_adjust;
	}
...
/**
 * Initialize shared memory for servo state communication between domains
 */
int clock_init_servo_shared_memory(struct clock *c)
{
	c->servo_shm = servo_state_shm_create();
	if (!c->servo_shm) {
		pr_err("Failed to create servo state shared memory");
		return -1;
	}
	return 0;
}
 ...
 */
void clock_destroy_servo_shared_memory(struct clock *c)
{
	if (c->servo_shm) {
		servo_state_shm_destroy(c->servo_shm);
		c->servo_shm = NULL;
	}
}

/**
 * Initialize shared memory for master restart detection communication between domains
 */
int clock_init_master_restart_shared_memory(struct clock *c)
{
	c->master_restart_shm = master_restart_shm_create();
	if (!c->master_restart_shm) {
		pr_err("Failed to create master restart detection shared memory");
		return -1;
	}
	return 0;
}
...
/**
 * Clean up master restart detection shared memory resources
 */
void clock_destroy_master_restart_shared_memory(struct clock *c)
{
	if (c->master_restart_shm) {
		master_restart_shm_destroy(c->master_restart_shm);
		c->master_restart_shm = NULL;
	}
}

/**
 * Initialize shared memory for slave servo stable state communication between domains
 */
int clock_init_slave_servo_stable_shared_memory(struct clock *c)
{
	c->slave_servo_stable_shm = slave_servo_stable_shm_create();
	if (!c->slave_servo_stable_shm) {
		pr_err("Failed to create slave servo stable shared memory");
		return -1;
	}
	return 0;
}
...
/**
 * Clean up slave servo stable shared memory resources
 */
void clock_destroy_slave_servo_stable_shared_memory(struct clock *c)
{
	if (c->slave_servo_stable_shm) {
		slave_servo_stable_shm_destroy(c->slave_servo_stable_shm);
		c->slave_servo_stable_shm = NULL;
	}
}

/**
 * Reset servo state to UNLOCKED when master connection is lost
 */
void clock_reset_servo_on_master_disconnect(struct clock *c)
{
	servo_reset(c->servo);
	c->servo_state = SERVO_UNLOCKED;
	
	/* Update shared memory if available and this is domain 0 */
	if (c->dds.domainNumber == 0 && c->servo_shm && (c->dds.flags & DDS_SLAVE_ONLY)) {
		servo_state_shm_update(c->servo_shm, SERVO_UNLOCKED, 0);
		pr_debug("*** Domain0-slave: Updated servo state to UNLOCKED in shared memory due to master disconnect ***");
	} else {
		pr_debug("*** Domain%d-slave: not updating shared memory ***", c->dds.domainNumber);
	}
}

void clock_request_servo_reset(struct clock *c)
{
	c->servo_reset_requested = 1;
	pr_debug("Servo reset requested due to sync interval exceed");
}

/**
 * Check if domain 0 master restart was detected
 */
int domain0_master_restart_detected(void)
{
		const char *pid_file = "/ptp4l_domain0_master.pid";
		FILE *fp;
		pid_t current_pid = getpid();
		pid_t file_pid = 0;
		int restart_detected = 0; /* return 1 represent restart detected, 0 represent not restart detected, -1 represent error */ 
		
		/* Try to read existing PID from file */
		fp = fopen(pid_file, "r");
		if (fp) {
			if (fscanf(fp, "%d", (int*)&file_pid) == 1) {
				/* File exists and contains valid PID */
				pr_debug("*** GM-Domain0-Master: Found existing PID file, file_pid=%d ***", file_pid);
				if (file_pid != current_pid) {
					restart_detected = 1;
					pr_info("*** GM-Domain0-Master: file_pid=%d, current_pid=%d ***", file_pid, current_pid);
				} else {
					pr_debug("*** GM-Domain0-Master: Same PID, no restart detected ***");
				}
			} else {
				pr_debug("*** GM-Domain0-Master: PID file exists but invalid format ***");
				restart_detected = 1; // Invalid file treated as restart
			}
			fclose(fp);
		} else {
			restart_detected = 1; // first starup be thought as restart
			pr_info("*** GM-Domain0-Master: first startup ***");
		}
		
		/* Always write current PID to file */
		fp = fopen(pid_file, "w");
		if (fp) {
			fprintf(fp, "%d\n", current_pid);
			fclose(fp);
			pr_debug("*** GM-Domain0-Master: Updated PID file with current_pid=%d ***", current_pid);
		} else {
			pr_err("Failed to write PID file %s: %m", pid_file);
			return -1;
		}
		
		pr_debug("*** GM-Domain0-Master: Restart detection result: %d ***", restart_detected);
		return restart_detected;
}

int clock_master_restart_detected(struct clock *c)
{
	return c->master_restart_detected;
}

int dds_domainNumber_get(struct clock *c)
{
	return c->dds.domainNumber;
}

struct slave_servo_stable_shm* slave_servo_stable_shm_get(struct clock *c)
{
	return c->slave_servo_stable_shm;
}

/**
 * Check if domain 1 master restart was detected
 */
int domain1_master_restart_detected(void)
{
	const char *pid_file = "/ptp4l_domain1_master.pid";
	FILE *fp;
	pid_t current_pid = getpid();
	pid_t file_pid = 0;
	int restart_detected = 0; /* return 1 represent restart detected, 0 represent not restart detected, -1 represent error */ 
	...
	if (fp) {
		if (fscanf(fp, "%d", (int*)&file_pid) == 1) {
			...
			if (file_pid != current_pid) {
				restart_detected = 1;
				pr_info("*** hotstanby-Domain1-Master: file_pid=%d, current_pid=%d ***", file_pid, current_pid);
			}
		}
		fclose(fp);
	} else {
		/* File doesn't exist */
		restart_detected = 1; // first starup be thought as restart
		pr_info("*** hotstanby-Domain1-Master: first startup ***");
	}
